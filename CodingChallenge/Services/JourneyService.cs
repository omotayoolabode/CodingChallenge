...
public class JourneyService : IJourneyService
{
    ...
    public async Task<List<Journey>> FindJourneysByMinDurationAsync(string origin, string destination, DateTime departure, bool ascending = true, int? maxExchanges = null, int maxResults = 100)
    {
        // Get routes from cache or load from database
        var routes = await _cache.GetOrCreateAsync(ROUTES_CACHE_KEY, async entry =>
        {
        ...
        results = ascending
            ? results.OrderBy(j => j.TotalDuration).Take(maxResults).ToList()
            : results.OrderByDescending(j => j.TotalDuration).Take(maxResults).ToList();
        return results;
    }

    public async Task<(List<Journey> Journeys, int TotalCount, int CurrentPage, int PageSize)> GetAllJourneysAsync(int page = 1, int size = 100, string orderBy = "NumberOfExchanges", bool ascending = true, int? maxExchanges = null)
    {
        // Get routes from cache or load from database
        var routes = await _cache.GetOrCreateAsync(ROUTES_CACHE_KEY, async entry =>
        {
        ...
        var allLocations = routes.Select(r => r.From)
            .Union(routes.Select(r => r.To))
            .Distinct()
            .ToList();
        ...
        var allJourneys = new List<Journey>();

        // Calculate all possible journeys between all location pairs
        foreach (var origin in allLocations)
        {
        ...
        // Apply ordering
        var orderedJourneys = orderBy.ToLower() switch
        {
            "numberofexchanges" => ascending ? allJourneys.OrderBy(j => j.NumberOfExchanges) : allJourneys.OrderByDescending(j => j.NumberOfExchanges),
            "totalprice" => ascending ? allJourneys.OrderBy(j => j.TotalPrice) : allJourneys.OrderByDescending(j => j.TotalPrice),
            "totalduration" => ascending ? allJourneys.OrderBy(j => j.TotalDuration) : allJourneys.OrderByDescending(j => j.TotalDuration),
            _ => ascending ? allJourneys.OrderBy(j => j.NumberOfExchanges) : allJourneys.OrderByDescending(j => j.NumberOfExchanges)
        };

        var totalCount = allJourneys.Count;
        var paginatedJourneys = orderedJourneys
            .Skip((page - 1) * size)
            .Take(size)
            .ToList();

        return (paginatedJourneys, totalCount, page, size);
    }
} 